...
from utils import PrintColors, RandomSearchMetaOptimizer, TensorboardLogger
from globals import GLOBAL_CONSTANTS
from flesh_model_s import AlternatingSeq2Seq

from allennlp.data import Vocabulary

reader = IrregularGenerationReader()
train_data, val_data = (reader.read(folder) for folder in
                        [GLOBAL_CONSTANTS.train_data_path, GLOBAL_CONSTANTS.val_data_path])
''' build vocabulary.  '''
vocabulary = Vocabulary.from_instances(train_data + val_data, non_padded_namespaces=['events'])
''' note that iterator is NOT associated with data. this project does not supports easy batching '''
iterator = AlternatingSeq2seqIterator(batch_size=1)
''' tell iterator how to numericalize input '''
iterator.index_with(vocabulary)

aa = reader.text_to_instance(event_context=['{}_grocery'.format(GLOBAL_CONSTANTS.beginning_event)],
                             text_context=['{}_grocery'.format(GLOBAL_CONSTANTS.begin_of_story)],
                             indices=[],
                             forthcoming_event='irregular_grocery')
gen = iterator([aa], num_epochs=1)
bb = list(gen)
