"""
sadly we have to hack this piece of code to track irregular losses
hack list:
    removed registry to avoid collision
    added self.loss_tracker
    update self.loss_tracker within validation loops
"""
...
class Validate(Callback):
    ...
    def __init__(self,
                 validation_data: Iterable[Instance],
                 validation_iterator: DataIterator,
                 loss_tracker: Callback = None) -> None:
            ...
            for batch_group in val_generator_tqdm:
                ...
                if self.loss_tracker is not None:
                    ...
                    if trainer.model._effective_encoder is trainer.model._sketch_encoder:
                        self.loss_tracker.cumulated_regular_loss += loss.detach().cpu().numpy()
                        self.loss_tracker.regular_batch_count += 1.
                    else:
                        self.loss_tracker.cumulated_irregular_loss += loss.detach().cpu().numpy()
